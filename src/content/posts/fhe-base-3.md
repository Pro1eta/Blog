---
title: 【同态加密的数学基础3】涉及理想的一些定理
published: 2026-03-04
description: 涉及理想的一些定理等
image: ""
tags:
  - Math
  - FHE
...
> **定义 23** （整除）
> 
> $R$ 是交换环。设 $a, b \in R$。若存在 $c \in R$ 使得 $b = ac$，则称 $a$ 整除 $b$（或 $b$ 被 $a$ 整除），记作 $a \mid b$。

:::important
方便起见，我的笔记中不区分左整除和右整除，将整除定义在交换环上。
:::

> **定义 24** （单位）
> 
> 设 $R$ 是含元交换环。若元素 $u \in R$ 存在逆元 $v \in R$ 使得 $uv = 1_R$，则称 $u$ 是 $R$ 中的一个单位（或可逆元）。$R$ 中全体单位构成的集合记作 $U(R)$。

> **定义 25** （相伴）
> 
> 设 $a, b \in R$。若存在单位 $u \in U(R)$ 使得 $a = ub$，则称 $a$ 与 $b$ 相伴，记作 $a \sim b$。在整环中，这等价于 $a \mid b$ 且 $b \mid a$。

:::note
在整环中，相伴等价于 $a \mid b$ 且 $b \mid a$。
:::

> **定义 26** （素元）
> 
> 设 $p \in R$ 且 $p \neq 0, p \notin U(R)$。若对于任意 $a, b \in R$，$p \mid ab$ 蕴含 $p \mid a$ 或 $p \mid b$，则称 $p$ 是 $R$ 中的一个素元。

> **定义 27** （不可约元）
> 
> 设 $q \in R$ 且 $q \neq 0, q \notin U(R)$。若 $q = ab$ 蕴含 $a \in U(R)$ 或 $b \in U(R)$，则称 $q$ 是 $R$ 中的一个不可约元。
...
若 $p$ 是素元且 $p=ab$，则 $p|ab \implies p|a$ 或 $p|b$。若 $p|a$，设 $a=pk=abk \implies 1=bk$（因为是整环可以约去 $a$），故 $b$ 是单位。
...
> PID 都是唯一分解整环 （UFD）。
> 
> 即：对于 PID 中的每个 $p$，如果 $p \neq 0$ 且不是单位，则可唯一分解为不可约元的乘积。
...
令 $I = \bigcup_{n=1}^\infty (a_n)$。在 PID 中，$I$ 是主理想，设 $I = (a)$。则 $a \in \bigcup (a_n) \implies \exists k$ 使得 $a \in (a_k)$，所以 $(a) \subseteq (a_k)$。

由于 $I$ 是并集，所以 $(a_n) \subseteq (a) \subseteq (a_k), n>k$，但这和上面的过程是矛盾的。

因此，每一个非零非单位的元素都可以分解为不可约元乘积。

假设某种元素有两种分解形式： $p_1 p_2 \dots p_n = q_1 q_2 \dots q_m$，其中 $p_i, q_j$ 都是不可约元。

PID 中不可约元 $p_1$ 是素元。由 $p_1 | q_1 q_2 \dots q_m \implies \exists q_j$ 使得 $p_1 | q_j$。因 $q_j$ 不可约，故 $p_1$ 与 $q_j$ 相伴。

消去该项并对因子个数进行归纳，即证在相伴意义下，分解是唯一的。